...
class Player(pygame.sprite.Sprite):
	def __init__(self, x, y):
		...
		for i in range(1,3):
			image = pygame.image.load(get_path(f'Assets/Player/PlayerIdle{i}.png'))
			image = pygame.transform.scale(image, (24, 24))
			self.idle_list.append(image)
		for i in range(1,6):
			image = pygame.image.load(get_path(f'Assets/Player/PlayerWalk{i}.png'))
			right = pygame.transform.scale(image, (24, 24))
			left = pygame.transform.flip(right, True, False)
			self.walk_right.append(right)
			self.walk_left.append(left)
		for i in range(1, 5):
			image = pygame.image.load(get_path(f'Assets/Player/PlayerAttack{i}.png'))
			image = pygame.transform.scale(image, (24, 24))
			self.attack_list.append(image)
		for i in range(1,11):
			image = pygame.image.load(get_path(f'Assets/Player/PlayerDead{i}.png'))
			image = pygame.transform.scale(image, (24, 24))
			self.death_list.append(image)
		for i in range(1, 3):
			image = pygame.image.load(get_path(f'Assets/Player/PlayerHit{i}.png'))
			image = pygame.transform.scale(image, (24, 24))
