...
# load images
sky = pygame.transform.scale(pygame.image.load(get_path('assets/BG1.png')), (SCREEN_WIDTH, SCREEN_HEIGHT))
mountain = pygame.transform.scale(pygame.image.load(get_path('assets/BG2.png')), (SCREEN_WIDTH, SCREEN_HEIGHT))
ground = pygame.transform.scale(pygame.image.load(get_path('assets/BG3.png')), (SCREEN_WIDTH, SCREEN_HEIGHT))

save_img = pygame.image.load(get_path('assets/save_btn.png'))
load_img = pygame.image.load(get_path('assets/load_btn.png'))
left_img = pygame.image.load(get_path('assets/left.png'))
right_img = pygame.image.load(get_path('assets/right.png'))

# load tileset

img_list = []
for i in range(1,NUM_TILES+1):
	img = pygame.image.load(get_path(f'Tiles/{i}.png'))
	img_list.append(img)
...
while running:
	...
	if save_button.draw(win):
		#save level data
		if os.path.exists(get_path(f'levels/level{current_level}_data')):
			with open(get_path(f'levels/level{current_level}_data'), 'rb') as pickle_in:
				data = pickle.load(pickle_in)
			with open(get_path(f'levels/level{current_level}_backup_data'), 'wb') as pickle_out:
				pickle.dump(data, pickle_out)

		pickle_out = open(get_path(f'levels/level{current_level}_data'), 'wb')
		pickle.dump(world_data, pickle_out)
		pickle_out.close()
	if load_button.draw(win):
		#load in level data
		if os.path.exists(get_path(f'levels/level{current_level}_data')):
			pickle_in = open(get_path(f'levels/level{current_level}_data'), 'rb')
			data = pickle.load(pickle_in)
