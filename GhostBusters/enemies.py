...
pygame.mixer.init()
bullet_fx = pygame.mixer.Sound(get_path('Sounds/ghost_shot.mp3'))
...
class Ghost(pygame.sprite.Sprite):
	def __init__(self, x, y, win):
		...
		for i in range(1,6):
			image = pygame.image.load(get_path(f'Assets/Ghost/Enemywalk{i}.png'))
			right = pygame.transform.scale(image, (self.size, self.size))
			left = pygame.transform.flip(right, True, False)
			self.walk_right.append(right)
			self.walk_left.append(left)
		for i in range(1, 3):
			image = pygame.image.load(get_path(f'Assets/Ghost/Enemyhit{i}.png'))
			image = pygame.transform.scale(image, (self.size, self.size))
			self.hit_list.append(image)
		for i in range(1,9):
			image = pygame.image.load(get_path(f'Assets/Ghost/Enemydead{i}.png'))
			image = pygame.transform.scale(image, (self.size, self.size))
