...
# Function to get correct path for resources
...
BG1 = pygame.transform.scale(pygame.image.load(get_path('assets/BG1.png')), (WIDTH, HEIGHT))
BG2 = pygame.transform.scale(pygame.image.load(get_path('assets/BG2.png')), (WIDTH, HEIGHT))
BG3 = pygame.transform.scale(pygame.image.load(get_path('assets/BG3.png')), (WIDTH, HEIGHT))
MOON = pygame.transform.scale(pygame.image.load(get_path('assets/moon.png')), (300, 220))

# FONTS ***********************************************************************

title_font = get_path("Fonts/Aladin-Regular.ttf")
instructions_font = get_path('Fonts/BubblegumSans-Regular.ttf')
# about_font = get_path('Fonts/DalelandsUncialBold-82zA.ttf')

ghostbusters = Message(WIDTH//2, HEIGHT//2 - 90, 90, "GhostBusters", title_font, (255, 255, 255), win)
left_key = Message(WIDTH//2, HEIGHT//2 - 90, 20, "Press left arrow key to go left", instructions_font, (255, 255, 255), win)
right_key = Message(WIDTH//2, HEIGHT//2 - 65, 20, "Press right arrow key to go right", instructions_font, (255, 255, 255), win)
up_key = Message(WIDTH//2, HEIGHT//2 - 40, 20, "Press up arrow key to jump", instructions_font, (255, 255, 255), win)
space_key = Message(WIDTH//2, HEIGHT//2 - 15, 20, "Press space key to shoot", instructions_font, (255, 255, 255), win)
g_key = Message(WIDTH//2, HEIGHT//2 + 10, 20, "Press g key to throw grenade", instructions_font, (255, 255, 255), win)
p_key = Message(WIDTH//2, HEIGHT//2 + 35, 20, "Press p key to pause game", instructions_font, (255, 255, 255), win)
game_won_msg = Message(WIDTH//2, HEIGHT//2 - 50, 30, "You have won the game!", instructions_font, (255, 255, 255), win)
game_complete_msg = Message(WIDTH//2, HEIGHT//2 - 10, 20, "You defeated all the ghosts!", instructions_font, (255, 255, 255), win)
paused_msg = Message(WIDTH//2, HEIGHT//2, 50, "PAUSED", title_font, (255, 255, 255), win)
resume_msg = Message(WIDTH//2, HEIGHT//2 + 50, 20, "Press P to resume", instructions_font, (255, 255, 255), win)
score_msg = Message(WIDTH - 100, 20, 18, "Score: 0", instructions_font, (255, 255, 255), win)
...
with open(get_path('Data/about.txt')) as f:
	info = f.read().replace('\n', ' ')

# BUTTONS *********************************************************************

ButtonBG = pygame.image.load(get_path('Assets/ButtonBG.png'))
...
pygame.mixer.music.load(get_path('Sounds/mixkit-complex-desire-1093.mp3'))
pygame.mixer.music.play(loops=-1)
pygame.mixer.music.set_volume(0.5)

diamond_fx = pygame.mixer.Sound(get_path('Sounds/point.mp3'))
diamond_fx.set_volume(0.6)
bullet_fx = pygame.mixer.Sound(get_path('Sounds/bullet.wav'))
jump_fx = pygame.mixer.Sound(get_path('Sounds/jump.mp3'))
health_fx = pygame.mixer.Sound(get_path('Sounds/health.wav'))
menu_click_fx = pygame.mixer.Sound(get_path('Sounds/menu.mp3'))
next_level_fx = pygame.mixer.Sound(get_path('Sounds/level.mp3'))
grenade_throw_fx = pygame.mixer.Sound(get_path('Sounds/grenade throw.wav'))
...
p_image = pygame.transform.scale(pygame.image.load(get_path('Assets/Player/PlayerIdle1.png')), (32,32))
...
while running:
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN:
			...
			if event.key == pygame.K_p and game_start and not game_won:
		...
		if event.type == pygame.KEYDOWN and game_start and not game_paused:
	...
	if main_menu:
		...
		game_won_msg.update()
		game_complete_msg.update()
		score_display = Message(WIDTH//2, HEIGHT//2 + 30, 25, f"Final Score: {score}", instructions_font, (255, 255, 255), win)
		...
		# Display score
		score_msg.update(f"Score: {score}")
